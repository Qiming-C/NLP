...
class WordRecallThresholdFactChecker(object):
    ...
    def _compute_similarity(self, fact_words: set, passage_words: set) -> float:
        if not fact_words or not passage_words:
            return 0.0

        # Calculate word overlap ratio
        overlap = len(fact_words.intersection(passage_words))
    ...
    def predict(self, fact: str, passages: List[dict]) -> str:
        ...
        # compute similarity for each passage
        max_similarity = 0.0
        for passage in passages:
            passage_words = self._preprocess_text(passage['text'])
            similarity = self._compute_similarity(fact_words, passage_words)
            max_similarity = max(max_similarity, similarity)

        # predict
        threshold = 0.65
        return "S" if max_similarity >= threshold else "NS"
